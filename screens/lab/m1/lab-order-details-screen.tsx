      ...
      >
          ...
          {order.items.map((item) => (
            <View key={item.id} className="mb-4 rounded-[24px] bg-white p-5 shadow-sm border border-slate-50">
              ...
              {isEstimationRequest ? (
                ...
                item.price && (
                  <View className="flex-row-reverse items-center justify-between border-t border-slate-50 pt-3">
        ...
        {isConfirmed && (
          <View className="mt-4 rounded-[32px] bg-blue-600 p-8 shadow-xl">
             ...
             <Pressable 
               className={`flex-row-reverse items-center justify-center gap-3 rounded-2xl bg-white py-4 ${submitting ? 'opacity-50' : ''}`}
