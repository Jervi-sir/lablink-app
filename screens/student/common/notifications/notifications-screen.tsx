  ...
  return (
    <ScreenWrapper style={{ backgroundColor: '#FFF' }}>
      ...
      <View style={{ paddingHorizontal: 16, paddingVertical: 16, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
        <Text style={{ fontSize: 24, fontWeight: '800', color: '#111' }}>Notifications</Text>
        <TouchableOpacity>
          <Text style={{ fontSize: 14, fontWeight: '700', color: '#137FEC' }}>Mark all read</Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ backgroundColor: '#F8F9FB', paddingBottom: 100 }}>
        {NOTIFICATIONS.map((section) => (
          <View key={section.section} style={{ paddingTop: 24 }}>
            <Text style={{ fontSize: 12, fontWeight: '800', color: '#6B7280', paddingHorizontal: 16, marginBottom: 12 }}>{section.section}</Text>
            <View style={{ paddingHorizontal: 16, gap: 12 }}>
              {section.data.map((item) => (
                <TouchableOpacity key={item.id} style={{ flexDirection: 'row', backgroundColor: '#FFF', borderRadius: 16, padding: 16, gap: 14, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.03, shadowRadius: 8, elevation: 2 }}>
                  ...
                  <View style={{ flex: 1, gap: 4 }}>
                    ...
                    <Text style={{ fontSize: 13, color: '#5D6575', lineHeight: 18, fontWeight: '500' }} numberOfLines={2}>{item.message}</Text>
                  </View>
                </TouchableOpacity>
              ))}
            </View>
      ...
      <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 80, backgroundColor: '#FFF', flexDirection: 'row', borderTopWidth: 1, borderTopColor: '#F0F2F5', paddingBottom: 20 }}>
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', gap: 4 }}>
          ...
          <Text style={{ fontSize: 11, fontWeight: '600', color: '#9CA3AF' }}>Home</Text>
        </View>
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', gap: 4 }}>
          <View style={{ width: 24, height: 24, backgroundColor: '#9CA3AF', borderRadius: 4 }} />
          <Text style={{ fontSize: 11, fontWeight: '600', color: '#9CA3AF' }}>Browse</Text>
        </View>
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', gap: 4 }}>
          ...
          <Text style={[{ fontSize: 11, fontWeight: '600', color: '#9CA3AF' }, { color: '#137FEC' }]}>Alerts</Text>
        </View>
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', gap: 4 }}>
          <View style={{ width: 24, height: 24, backgroundColor: '#9CA3AF', borderRadius: 4 }} />
          <Text style={{ fontSize: 11, fontWeight: '600', color: '#9CA3AF' }}>Profile</Text>
        </View>
      </View>
    </ScreenWrapper>
  );
